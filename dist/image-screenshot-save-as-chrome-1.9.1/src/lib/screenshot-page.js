export function preparePageForScreenshot(recoveryTimeoutMs) {
	...
	function scheduleRecovery() {
		const state = JSON.parse(doc.getAttribute(recoveryStateAttribute));
		state.recoveryTimeoutMs = Math.max(10_000, Number(recoveryTimeoutMs) || 120_000);
		state.recoveryDeadline = Date.now() + state.recoveryTimeoutMs;
		doc.setAttribute(recoveryStateAttribute, JSON.stringify(state));
		window.setTimeout(checkRecovery, state.recoveryTimeoutMs);
		...
		function checkRecovery() {
			...
			if (current.recoveryToken !== recoveryToken) return;
			const remaining = current.recoveryDeadline - Date.now();
			if (remaining > 0) {
				window.setTimeout(checkRecovery, remaining);
			} else {
				restoreStoredCaptureState(recoveryToken);
			}
		}
...
export function scrollPageForScreenshot(state, scrollY) {
	...
	// Renew only from capture work, never from the recovery timer itself.
	storedState.recoveryDeadline = Date.now() + storedState.recoveryTimeoutMs;
	document.documentElement.setAttribute(
		"data-img-save-as-capture-recovery", JSON.stringify(storedState),
	);
