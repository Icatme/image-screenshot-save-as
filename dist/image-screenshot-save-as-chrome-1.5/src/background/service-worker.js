...
async function captureFullPageScreenshotBlob(tab, format, settings) {
	...
	try {
		...
		if (pageState) {
			await executeTabFunction(tab.id, restorePageAfterScreenshot, [
				pageState,
			]).catch(() => {});
		}
	}
...
async function fetchImageBlob(srcUrl) {
	// Credentials are scoped by the browser to the requested origin. Including
	// them here preserves support for authenticated image hosts and CDNs.
...
async function startDownload(objectUrl, filename, saveAs) {
	...
	try {
		...
		return {
			...
			promptLikelyBlocked:
				downloadId == null &&
				Date.now() - startedAt <= SAVE_AS_PROMPT_BLOCKED_MS,
		};
	} catch (error) {
...
async function queuePendingDownload(downloadId, payload) {
	...
	await chrome.storage.session.set({
		[PENDING_DOWNLOADS_KEY]: pendingDownloads,
	});
}
...
async function appendSaveHistory(entry) {
	...
	const current = Array.isArray(stored[SAVE_HISTORY_KEY])
